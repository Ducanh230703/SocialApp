...
namespace Services
{
    ...
    {
        public static async Task<ApiReponseModel> dangxuat(int userId)
        {
            var sql = $"UPDATE Users SET IsOnline = 0 WHERE {userId}";

            var rs = await connectDB.Update(sql);

            if (rs == 1) return new ApiReponseModel
            {
                Status = 1,
                Mess = "Đăng xuất thành công."
            };
            else return new ApiReponseModel
            {
                Status = 0,
        ...
        /// <returns></returns>
        public static async Task<ApiReponseModel<UserReponseModel>> UserRegister(string Email, string Password, string FullName)
        {
            ...
            var sql = "INSERT INTO Users (Email, Password, FullName) VALUES (@Email, @PasswordHash, @FullName)";
            ...
            if (rs ==1) return new ApiReponseModel<UserReponseModel>
            {
                Status = 1,
                Mess = "Đăng ký thành công."
            };
            else return new ApiReponseModel<UserReponseModel>
            {
                Status = 0,
        ...
        /// <returns></returns>
        public static async Task<ApiReponseModel<UserReponseModel>> GetUserInfo(string token)
        {
            var user = Cache.CacheEx.GetUserFromToken(token);
            ...
            if (user == null)
            {
                ...
                {
                    Status = 0,
                    Mess = "Token không hợp lệ hoặc người dùng không tồn tại.",
                    Data = null
                };
            }
        ...
        public static async Task<PaginatedResponse<PostFull>> GetUserById (int pageNumber, int pageSize,int UserId)
        {
            ...
            string sql = $@"SELECT
                                ...
                                (
                                    -- CẬP NHẬT Ở ĐÂY: Sử dụng STRING_AGG và JSON_QUERY để tạo mảng số nguyên
        ...
        public static async Task<List<ListFriend>> GetFriendByUserId(int UserId)
        {
            var sql = @"SELECT
                                CASE
                                    WHEN FS.SenderId = @YourUserId THEN U_Receiver.FullName
                                    ELSE U_Sender.FullName
                                END AS FriendFullName
                            FROM
                                Friendships AS FS
                            JOIN
                                Users AS U_Sender ON FS.SenderId = U_Sender.ID
                            JOIN
                                Users AS U_Receiver ON FS.ReceiverId = U_Receiver.ID
                            WHERE
        ...
        public static async Task<ApiReponseModel<UserInfo>> GetUserInfoWithPaginatedPosts(int profileUserId, string token, int pageNumber = 1, int pageSize = 10)
        {
            var userInfo = new UserInfo();

            var userDetails = await GetUserInfo(token);
            if (userDetails != null)
            {
            ...
            userInfo.ListPost = await GetUserById(profileUserId, pageNumber, pageSize);
